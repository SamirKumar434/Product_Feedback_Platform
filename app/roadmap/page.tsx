...
import { Badge, BarChart3, CheckCheck, Clock, Target } from "lucide-react";
        ...
        {STATUS_ORDER.map((status) => {
          ...
          return (
            <div key={status} className="space-y-4">
              <div
                className={`flex items-center justify-between p-3 rounded-lg border ${group.color}`}
              >
              ...
              <div className="space-y-3">
                {postInGroup.map((post) => (
                  <Card
                    key={post.id}
                    className={`border-l-4 ${group.color} hover:shadow-lg transition-all`}
                  >
